...
#
# docker build . -t debian-xfce -f debian-xfce.dockerfile
# docker run -d --name debian-xfce -p 6080:6080 -p 5901:5901 debian-xfce
...
RUN apt-get update -y && apt-get install -y --no-install-recommends \
    xfce4 xfce4-terminal mousepad thunar xfwm4 \
    && apt-get clean && rm -rf /var/lib/apt/lists/*
